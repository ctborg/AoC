let sol = `16-18 h: hhhhhhhhhhhhhhhhhh
...
let sols = sol.split(/\r?\n/)

let numMatches = 0;
sols.forEach(i => {
  let line = i.split(' ');
  
  let [passMin, passMax] = line[0].split('-');
  let charMatch = line[1].replace(':','');
  let pass = line[2];
  
  let firstMatch = pass.charAt(passMin - 1);
  let secondMatch = pass.charAt(passMax- 1);  
  
  if(firstMatch == charMatch || secondMatch == charMatch) {
    if (firstMatch == charMatch && secondMatch == charMatch) {
